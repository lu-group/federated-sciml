...
for iter in range(5):
    ...
    for i in range(len(n_pieces)):
        ...
        for i in range(global_epochs):
            ...
            if i==0 or (i+1)%1000 == 0:
                ...
                # fc4.append(np.linalg.norm(Wfedavg['branch.linears.3.weight'].cpu().numpy() - WSGD['branch.linears.3.weight'].cpu().numpy()) / np.linalg.norm(WSGD['branch.linears.3.weight'].cpu().numpy()))
                fc5.append(np.linalg.norm(Wfedavg['trunk.linears.0.weight'].cpu().numpy() - WSGD['trunk.linears.0.weight'].cpu().numpy()) / np.linalg.norm(WSGD['trunk.linears.0.weight'].cpu().numpy()))
                fc6.append(np.linalg.norm(Wfedavg['trunk.linears.1.weight'].cpu().numpy() - WSGD['trunk.linears.1.weight'].cpu().numpy()) / np.linalg.norm(WSGD['trunk.linears.1.weight'].cpu().numpy()))
                fc7.append(np.linalg.norm(Wfedavg['trunk.linears.2.weight'].cpu().numpy() - WSGD['trunk.linears.2.weight'].cpu().numpy()) / np.linalg.norm(WSGD['trunk.linears.2.weight'].cpu().numpy()))
                # fc8.append(np.linalg.norm(Wfedavg['trunk.linears.3.weight'].cpu().numpy() - WSGD['trunk.linears.3.weight'].cpu().numpy()) / np.linalg.norm(WSGD['trunk.linears.3.weight'].cpu().numpy()))
    ...
    np.savez("burgers{}.npz".format(iter), loss = loss)
    np.savez("burgers{}_wd.npz".format(iter), 
             fc1 = wd_fc1, fc2 = wd_fc2, fc3 = wd_fc3, 
             fc5 = wd_fc5, fc6 = wd_fc6, fc7 = wd_fc7)
