...
for iter in range(5):
    ...
    for client_pi_phase in client_pi_phases:
        ...
        # Server model
        # server_net = dde.nn.DeepONetCartesianProd([101, 40, 40], [2, 40, 40], "relu", "Glorot normal")
        server_net = dde.nn.DeepONetCartesianProd([101,64,64,64], [2, 64,64,64], "relu", "Glorot normal")
        data = dde.data.TripleCartesianProd(X_train = X_train_clients[0], y_train=y_train_clients[0], X_test=X_test, y_test=y_test)
        ...
        for i in range(n_clients):
            # client_nets.append(dde.nn.DeepONetCartesianProd([101, 40, 40], [2, 40, 40], "relu", "Glorot normal"))
        ...
        loss.append(l2)
        torch.save(server_model.state_dict(), f"fed_model_phi{client_pi_phase}.pth")

    np.savez(f"burgers_{iter}_fed_10w.npz", loss = loss)
